...
function setWebSocketOnEvent(webSocket) {
    ...
    webSocket.addEventListener('message', async (event) => {
        try {
            ...
            console.log(data)
            if (data.command_type == "G1") {
                //console.log(data.response)
                var strValue = data.response.split('\r');
                BatteryvoltageTxt.value = strValue[1];
                //BatteryCapacitypercentageTxt.value = strValue[2];
                //BatteryTimeRemainingTxt.value = strValue[3];
                BatterycurrentTxt.value = strValue[4];
                //TemperatureTxt.value = strValue[5];
                I_PfrequencyTxt.value = strValue[6];
                FrequencyofBypassTxt.value = strValue[7];
                O_PfrequencyTxt.value = strValue[8];
                //console.log(strValue);
                //0.5초 딜레이 후 "F" 요청 보내기
                setTimeout(() => {
                    requestServerData("G3");
                }, 500);
            }
            if (data.command_type == "G3") {
                var strValue = data.response.replace('/\r\n/g', '\n');
                var strArray = strValue.split('\n');
                if (strArray.length > 0) {
                    strArray.splice(0, 1); // 첫 번째 요소 삭제
                }
                I_PVoltageTxt.value = strArray[0];
                BypassVoltageTxt.value = strArray[3];
                O_PVoltageTxt.value = strArray[6];
                LoadingPercentateTxt.value = strArray[9];
...
document.getElementById('getServerRealTimeData').addEventListener('click', (e) => {
    let commandData = `G1`;
    let data = JSON.stringify({ 'command_type': commandData });
    if (webSocket.readyState === webSocket.OPEN) {
        //sendString = data;
